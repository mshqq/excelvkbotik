...
keyboardUnSub = VkKeyboard(one_time=True)
keyboardUnSub.add_button('Сегодня', color=VkKeyboardColor.POSITIVE)
keyboardUnSub.add_line()
keyboardUnSub.add_button('Завтра', color=VkKeyboardColor.SECONDARY)
keyboardUnSub.add_line()
keyboardUnSub.add_button('Послезавтра', color=VkKeyboardColor.SECONDARY)
keyboardUnSub.add_line()
keyboardUnSub.add_button('Выбор по дню недели', color=VkKeyboardColor.PRIMARY)
keyboardUnSub.add_line()
keyboardUnSub.add_button('Подписаться на рассылку', color=VkKeyboardColor.POSITIVE)

#объявляем клавиатуру главного меню с рассылкой
keyboardSub = VkKeyboard(one_time=True)
keyboardSub.add_button('Сегодня', color=VkKeyboardColor.POSITIVE)
keyboardSub.add_line()
keyboardSub.add_button('Завтра', color=VkKeyboardColor.SECONDARY)
keyboardSub.add_line()
keyboardSub.add_button('Послезавтра', color=VkKeyboardColor.SECONDARY)
keyboardSub.add_line()
keyboardSub.add_button('Выбор по дню недели', color=VkKeyboardColor.PRIMARY)
keyboardSub.add_line()
...
def start():
    ...
    if localstate is True:
        keyboardLocal = keyboardSub.get_keyboard()
    else:
        keyboardLocal = keyboardUnSub.get_keyboard()
    vk.messages.send(
        user_id=event.user_id,
        random_id=get_random_id(),
        message="Вас приветствует бот с расписанием для 10А класса\nВоспользуйтесь клавиатурой для выбора дня",
        keyboard=keyboardLocal
    )
...
def deleteSub(user_id):
    ...
    if checkSub(user_id=event.user_id) is True:
        ...
        vk.messages.send(
            ...
            keyboard=keyboardUnSub.get_keyboard()
        )
    else:
        vk.messages.send(
            user_id=event.user_id,
...
thr1 = threading.Thread(target=scheduleRunner).start()
